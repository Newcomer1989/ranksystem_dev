...
function addon_channelinfo_toplist(&$addons_config,$ts3,$mysqlcon,$cfg,$dbname,$lang,&$db_cache) {
	...
	// individual smarty plugins
	$smarty->registerPlugin(
		\Smarty\Smarty::PLUGIN_MODIFIER,
		'duration_format',
		function ($value, $format = '%Dd, %hh, %mm, %ss') {
			if (is_numeric($value)) {
				$seconds = (int)$value;
			} else {
				$s = (string)$value;
				// searches for old format 565d 18h 23m 26s
				preg_match('/(\d+)\s*d/i', $s, $md);
				preg_match('/(\d+)\s*h/i', $s, $mh);
				preg_match('/(\d+)\s*m(?!s)/i', $s, $mm);
				preg_match('/(\d+)\s*s/i', $s, $ms);

				$days    = isset($md[1]) ? (int)$md[1] : 0;
				$hours   = isset($mh[1]) ? (int)$mh[1] : 0;
				$minutes = isset($mm[1]) ? (int)$mm[1] : 0;
				$secs    = isset($ms[1]) ? (int)$ms[1] : 0;
			...
			$totalHours   = intdiv($seconds, 3600);
	...
	if(isset($addons_config['channelinfo_toplist_active']['value']) && $addons_config['channelinfo_toplist_active']['value'] == '1') {
		if($addons_config['channelinfo_toplist_lastupdate']['value'] < ($nowtime - $addons_config['channelinfo_toplist_delay']['value'])) {
			...
			for ($nr = 0; $nr < 10; $nr++) {
				...
				$active_day = round($userdata[$nr]['count_day']) - round($userdata[$nr]['idle_day']);
				$smarty->assign('CLIENT_ACTIVE_TIME_LAST_DAY_'.($nr + 1),((new DateTime("@0"))->diff(new DateTime("@".$active_day))->format($cfg['default_date_format'])));
				$smarty->assign('CLIENT_ONLINE_TIME_LAST_DAY_'.($nr + 1),((new DateTime("@0"))->diff(new DateTime("@".round($userdata[$nr]['count_day'])))->format($cfg['default_date_format'])));
				$smarty->assign('CLIENT_IDLE_TIME_LAST_DAY_'.($nr + 1),((new DateTime("@0"))->diff(new DateTime("@".round($userdata[$nr]['idle_day'])))->format($cfg['default_date_format'])));
