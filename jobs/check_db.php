...
[SIZE=6]Updated: {$LAST_UPDATE_TIME}[/SIZE]', ENT_QUOTES);
		...
		if(version_compare($cfg['version_current_using'], '1.3.25', '<')) {
			if($mysqlcon->exec("CREATE TABLE IF NOT EXISTS `$dbname`.`addon_winner_toplist` (
				`uuid` CHAR(28) CHARACTER SET utf8 COLLATE utf8_unicode_ci,
				`name` VARCHAR(30) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
				`timestamp` int(10) UNSIGNED NOT NULL default 0,
				`award` TINYINT UNSIGNED NOT NULL default 0,
				`count` DECIMAL(14,3) NOT NULL default '0',
				`idle` DECIMAL(14,3) NOT NULL default '0',
				PRIMARY KEY (`uuid`,`timestamp`,`award`)
			);") === false) {
				$err_msg .= $lang['isntwidbmsg'].$mysqlcon->errorCode()." ".print_r($mysqlcon->errorInfo(), true).'<br>'; $err_lvl = 2;
				enter_logfile(2,"    [1.3.25] Created new table addon_winner_toplist failed. SQL error: ".$mysqlcon->errorCode()." ".print_r($mysqlcon->errorInfo(), true));
			} else {
				enter_logfile(4,"    [1.3.25] Created new table addon_winner_toplist successfully.");
			}

			if(($check_new_columns = $mysqlcon->query("SHOW COLUMNS FROM `$dbname`.`addon_winner_toplist` WHERE `field`='count'")->fetchAll(PDO::FETCH_ASSOC)) === false) { } else {
				if(!isset($check_new_columns[0]['Field'])) {
					if($mysqlcon->exec("ALTER TABLE `$dbname`.`addon_winner_toplist` ADD COLUMN `count` DECIMAL(14,3) NOT NULL default '0', ADD COLUMN `idle` DECIMAL(14,3) NOT NULL default '0';") === false) { } else {
						enter_logfile(4,"    [1.3.25-beta-3] Adjusted table addon_winner_toplist successfully.");
					} 
				}
			}

			if($mysqlcon->exec("INSERT IGNORE INTO `$dbname`.`addons_config` (`param`,`value`) VALUES
			('winner_toplist_active','0'),
			('winner_toplist_day_week',''),
			('winner_toplist_time_week',''),
			('winner_toplist_day_month',''),
			('winner_toplist_time_month',''),
			('winner_toplist_group_mode',''),
			('winner_toplist_group_week',''),
			('winner_toplist_group_month','');") === false) { } else {
				enter_logfile(4,"    [1.3.25] Adjusted table addons_config successfully.");
			}
