...
$addons_config = load_addons_config($mysqlcon,$lang,$cfg,$dbname);
	if(isset($addons_config['assign_groups_active']['value']) && $addons_config['assign_groups_active']['value'] === '1') {
	...
	if(isset($addons_config['winner_toplist_active']['value']) && $addons_config['winner_toplist_active']['value'] === '1') {
		enter_logfile(4,"  Addon: 'winner_toplist' [ON]");
		include(dirname(__DIR__).DIRECTORY_SEPARATOR.'jobs/addon_winner_toplist.php');
		$cfg['temp_addon_winner_toplist'] = "enabled";
	} else {
		enter_logfile(4,"  Addon: 'winner_toplist_active' [OFF]");
		$cfg['temp_addon_winner_toplist'] = "disabled";
	}
...
function run_bot(&$cfg) {
	...
	try {
		...
		while(1) {
			...
			if(isset($addons_config['winner_toplist_active']['value']) && $addons_config['winner_toplist_active']['value'] === '1') {
				$sqlexec .= addon_winner_toplist($addons_config,$ts3server,$mysqlcon,$cfg,$dbname,$lang,$db_cache);
			}
